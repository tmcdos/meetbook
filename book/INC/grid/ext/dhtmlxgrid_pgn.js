...
===================================================================
*/
dhtmlXGridObject.prototype.lL = function (a, b, c, d, e, g)
{
...
dhtmlXGridObject.prototype.eG = function (a, b)
{
  this.tR = a;
  this.Cr = b
};
...
{
  this.changePage(this.ew + a)
};

dhtmlXGridObject.prototype.changePage = function (a)
{
  arguments.length == 0 && (a = this.ew || 0);
  a = parseInt(a);
  a = Math.max(1, Math.min(a, Math.ceil(this.aD.length / this.cu)));
  if (this.callEvent("onBeforePageChanged", [this.ew, a])) this.ew = parseInt(a), this._reset_view(), this._fixAlterCss(), this.callEvent("onPageChanged", this.ajA())
};
...
dhtmlXGridObject.prototype._page_skin_update = function ()
{
  ...
  var a = Math.ceil(this.aD.length / this.cu);
  if (a && a < this.ew) return this.changePage(a);
  this.aW && this._pgn_skin && this._pgn_skin.apply(this, this.ajA())
};
...
{
  for (var b = Math.floor((this.ew - 1) / this.pagesInGroup) * this.pagesInGroup, c = Math.min(Math.ceil(this.aD.length / this.cu), b + this.pagesInGroup), d = [], e = b + 1; e <= c; e++) 
    e == this.ew ? d.push("<a class='dhx_not_active'><b>" + e + "</b></a>") : d.push("<a onclick='this.grid.changePage(" + e + "); return false;'>" + e + "</a>");
  return d.join(a)
};

dhtmlXGridObject.prototype._pgn_link = function (a, b, c)
{
  if (a == "prevpages" || a == "prev") return this.ew == 1 ? c : "<a onclick='this.grid.changePageRelative(-1*" + (a == "prev" ? "1" : "this.grid.pagesInGroup") + "); return false;'>" + b + "</a>";
  if (a == "nextpages" || a == "next") return this.aD.length / this.cu <= this.ew ? c : this.aD.length / (this.cu * (a == "next" ? 1 : this.pagesInGroup)) <= 1 ? c : "<a onclick='this.grid.changePageRelative(" + (a == "next" ? "1" : "this.grid.pagesInGroup") + "); return false;'>" + b + "</a>";
  if (a == "current")
  {
    var d = this.ew + (b ? parseInt(b) : 0);
    return d < 1 || Math.ceil(this.aD.length / this.cu) < d ? c : "<a " + (d == this.ew ? "class='dhx_active_page_link' " : "") + "onclick='this.grid.changePage(" + d + "); return false;'>" + d + "</a>"
  }
  return b
};
...
dhtmlXGridObject.prototype._pgn_bricks = function (a, b, c)
{
  ...
  if (this.aD.length > 0)
  {
    ...
    if (Math.ceil(a / this.pagesInGroup) * this.pagesInGroup < Math.ceil(this.aD.length / this.cu)) f = document.createElement("DIV"), h = document.createElement("DIV"), f.className = "dhx_page" + e, h.innerHTML = "&rarr;", f.appendChild(h), this.pagerElAr.pagerBox.appendChild(f), i = this, f.pgnum = Math.ceil(a / this.pagesInGroup) * this.pagesInGroup + 1, f.onclick = function ()
    {
      i.changePage(this.pgnum)
    }, j += k;
    this.pagerElAr.pagerLine.style.width = j + "px"
  }
  if (this.aD.length > 0 && this.showRecInfo) this.pagerElAr.pagerInfoBox.innerHTML = this.i18n.paging.records + (b + 1) + this.i18n.paging.to + c + this.i18n.paging.of + this.aD.length;
  else if (this.aD.length == 0) this.pagerElAr.pagerLine.parentNode.removeChild(this.pagerElAr.pagerLine), this.pagerElAr.pagerInfoBox.innerHTML = this.i18n.paging.notfound;
...
dhtmlXGridObject.prototype._pgn_toolbar = function (a, b, c)
{
  ...
  var d = Math.ceil(this.aD.length / this.cu);
  this._WTDef[0] && (this.aToolBar.enableItem("right"), this.aToolBar.enableItem("rightabs"), this.aToolBar.enableItem("left"), this.aToolBar.enableItem("leftabs"), this.ew >= d && (this.aToolBar.disableItem("right"), this.aToolBar.disableItem("rightabs")), this.ew == 1 && (this.aToolBar.disableItem("left"), this.aToolBar.disableItem("leftabs")));
  if (this._WTDef[2])
  {
    var e = this;
  ...
  this._WTDef[1] && (this.iD() ? this.aToolBar.setItemText("results", "<div style='width:100%; text-align:center'>" + this.i18n.paging.records + (b + 1) + this.i18n.paging.to + c + "</div>") : this.aToolBar.setItemText("results", this.i18n.paging.notfound));
  this._WTDef[3] && this.aToolBar.setItemText("perpagenum", "<div style='width:100%; text-align:right'>" + this.cu.toString() + " " + this.i18n.paging.perpage + "</div>");
  this.callEvent("onPaging", [])
};
...
dhtmlXGridObject.prototype._pgn_createToolBar = function ()
{
  ...
  this._WTDef[0] && (this.aToolBar.addButton("leftabs", NaN, "", this.eg + "ar_left_abs.gif", this.eg + "ar_left_abs_dis.gif"), this.aToolBar.setWidth("leftabs", "20"), this.aToolBar.addButton("left", NaN, "", this.eg + "ar_left.gif", this.eg + "ar_left_dis.gif"), this.aToolBar.setWidth("left", "20"));
  ...
  this._WTDef[0] && (this.aToolBar.addButton("right", NaN, "", this.eg + "ar_right.gif", this.eg + "ar_right_dis.gif"), this.aToolBar.setWidth("right", "20"), this.aToolBar.addButton("rightabs", NaN, "", this.eg + "ar_right_abs.gif", this.eg + "ar_right_abs_dis.gif"), this.aToolBar.setWidth("rightabs", "20"));
